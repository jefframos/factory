...
/** Test trigger: a static, non-solid zone off to the side — see setupTriggerZone(). Walking into it fires onTriggerEnter/Stay/Exit on both the player's and the zone's own RigidBody, but never blocks movement (contrast with TEST_BOX_*, which is solid). Offset sideways rather than further down +Z so it's reachable without detouring around the solid box. */
const TRIGGER_ZONE_HALF_EXTENTS = new THREE.Vector3(1, 0.75, 1);
const TRIGGER_ZONE_OFFSET = new THREE.Vector3(3, 0, 3);
...
export default class PizzaScene extends ThreeScene {
    ...
    /** Same idea as setupTestBox(), but isTrigger:true — the player walks straight through it (no push-out) while its RigidBody still fires onTriggerEnter/Stay/Exit on both sides. See MainPlayer.registerCollisionEvents() for where the player's own side of that gets logged. */
    private setupTriggerZone(): void {
        const zone = this.world.spawn();
        zone.transform.position.copy(TRIGGER_ZONE_OFFSET);

        zone.addComponent(new RigidBody({
            halfExtents: TRIGGER_ZONE_HALF_EXTENTS,
            isStatic: true,
            isTrigger: true,
            layer: Layers.Trigger,
            centerOffset: new THREE.Vector3(0, TRIGGER_ZONE_HALF_EXTENTS.y, 0),
        }));
        zone.addComponent(new BoxVisualComponent(
            TRIGGER_ZONE_HALF_EXTENTS.clone().multiplyScalar(2),
            0x9b30ff,
            new THREE.Vector3(0, TRIGGER_ZONE_HALF_EXTENTS.y, 0),
        ));
