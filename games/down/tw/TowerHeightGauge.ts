...
export interface HeightMark {
     ...
     * FaceTowerGameController.getLevelProgressFraction()).
 ...
 * Dimmer, smaller ticks mark every milestone (completed trapdoor) already
 ...
 *
 * A third tick marks the fixed top GAME-OVER line (see
 * FaceTowerConfig.gameOverLineScreenY) — unlike the old per-zone "target"
 * line this used to show, it never moves: trapdoors only ever buy more
 * room below it. Milestone ticks (completed trapdoors) are dimmer/smaller
 * and not eased or top-clamped the same way — once passed they're
 * historical, so letting one scroll off the top just means it's no longer
 * the frontier.
 */
export class TowerHeightGauge {
    ...
    public constructor(root: PIXI.Container) {
        ...
        this.gameOverLine = new PIXI.Graphics();
        root.addChild(this.gameOverLine);
        ...
        this.gameOverLabel.anchor.set(1, 0.5);
        root.addChild(this.gameOverLabel);
    ...
    /**
     * `current` is the tower's current top; `gameOverLineScreenY` is the
     * fixed top game-over line (see FaceTowerConfig.gameOverLineScreenY —
     * never moves, trapdoors only ever buy more room below it);
     * `milestones` is every completed trapdoor's own fixed height. `current`/
     * `milestones` are already converted to this container's local screen
