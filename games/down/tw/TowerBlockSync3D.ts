...
export class TowerBlockSync3D {
    ...
    public sync(blocks: readonly FaceTowerBlock[], heldBlock: FaceTowerBlock | undefined, delta: number): void {
        ...
        for (const block of blocks) {
            ...
            this.updatePieceAnim(block.id, cube, delta);
        }
    ...
    /**
     * Applied after updateCube()'s position/rotation.z assignment so both
     * layer on top instead of being overwritten by it.
     */
    private updatePieceAnim(blockId: number, cube: THREE.Mesh, delta: number): void {
        ...
        const scaleX = shoot.scaleX * jiggle.scaleX;
        const scaleY = shoot.scaleY * jiggle.scaleY;
