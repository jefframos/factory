...
export class FaceTowerInputController {
    ...
    public constructor(
        ...
        /** See FaceTowerConfig.tapMovesPieceOnDrop's own doc. */
        private readonly moveToTapOnPointerDown: boolean,
    ) {
    ...
    private onPointerDown(
        ...
        /*
         * Gated on moveToTapOnPointerDown (see FaceTowerConfig.
         * tapMovesPieceOnDrop's own doc) — testers were split on which
         * felt right, so both stay available instead of picking one:
         *
         * true (default): repositions the held piece under the pointer
         * immediately, even on a plain tap with no drag — the original
         * behavior.
         *
         * false: a tap-and-release with no intervening pointermove leaves
         * the piece exactly where it already was; only an actual
         * swipe/drag repositions it (via onPointerMove, unaffected either
         * way). Matters most on touch, where a pointerdown is often the
         * FIRST event at all (no prior hover) — moving to it
         * unconditionally on a plain tap visibly snaps the piece sideways
         * to wherever the tap landed right before dropping it.
         */
        if (this.moveToTapOnPointerDown) {
