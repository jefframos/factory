...
export class TowerVfxUtils {
    ...
    /**
     * A merge landing its score pop — see TowerScorePopupUtils.popAt (the
     * 2D flying-number counterpart) in IslandViewScene's onMerge handler.
     * Takes raw 2D physics world coords (the merged pair's own midpoint)
     * rather than a block, since the top-tier + top-tier despawn case has
     * no resulting block to anchor the burst to.
     */
    public static onScorePopVfx(x: number, y: number): void {
        const cfg = DEFAULT_FACE_TOWER_CONFIG;
        const cfg3d = DEFAULT_TOWER_3D_CONFIG;
