...
export interface LevelZoneConfig {
    /** Total board weight (see FaceTowerBlockController.getTotalWeight/TowerZoneController) this zone's own trapdoor milestone requires — monotonically increasing across levels/zones the same way a plain climbed height used to. */
    weight: number;
    /** Fraction (0..1) of trapdoorDropHeight the containment poles stand at the current floor — see TowerDeadZoneController.rebuild(). Unrelated to `weight` — still just a wall-height knob. */
    polePercent: number;
}
