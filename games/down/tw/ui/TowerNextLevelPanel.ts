...
/**
 * Next-milestone progress readout — "current/target" board weight (e.g.
 * "18/24") over a horizontal fill bar, both driven by update(). Used to
 * show a climbed-height fraction before the trapdoor rework; repurposed to
 * the weight milestone that now actually gates the trapdoor (see
 * FaceTowerGameController.getTotalWeight/getTargetWeight/getWeightProgress)
 * — same widget, new source values. A separate container from TowerHeader
 * (which just shows "Level N") so GameHud can position each independently —
 * see PowerupBelt/TowerHeader for the same "shared background texture, own
...
export class TowerNextLevelPanel extends PIXI.Container {
    ...
    /** `currentWeight`/`targetWeight` are the board's current/needed total weight (see FaceTowerGameController.getTotalWeight/getTargetWeight) — plain whole numbers, no unit suffix. `fraction` (0..1) drives the fill. */
    public update(currentWeight: number, targetWeight: number, fraction: number): void {
        this.label.text = `${Math.round(currentWeight)}/${Math.round(targetWeight)}`;
