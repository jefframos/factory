...
export interface Tower3DConfig {
    ...
    // Purely cosmetic overrides for the trapdoor flap's OWN 3D mesh (see
    // TowerBaseSync3D.createPanel()) — independent from FaceTowerConfig's
    // floorHeight (still the real physics slab thickness, and still what
    // wall/dead-zone placement math is derived from) and from
    // platformDepth above (still shared by the side poles' own mesh). Lets
    // the flap's visual height/depth be tuned freely without shifting
    // anything about wall containment or the poles' look.
    trapdoorMeshHeight: number;
    trapdoorMeshDepth: number;
