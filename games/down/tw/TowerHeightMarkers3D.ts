...
export class TowerHeightMarkers3D {
    ...
    /**
     * `currentWorldY` is a 2D physics world Y value (same source
     * IslandViewScene.update() derives from
     * FaceTowerGameController.getCurrentTopWorldY() for the 2D gauge).
     * `targetWorldY` — despite the name kept for signature stability — is
     * now the fixed top game-over line's own world Y (see
     * FaceTowerGameController.getGameOverLineWorldY()), not a per-zone
     * height goal; its bar has no label (always hidden, see the
     * unconditional `goalLabel.setVisible(false)` below), so `targetValue`
     * is effectively unused. `currentValue` is the SAME already-scaled
     * number the 2D HUD shows — a level's own distance × climb progress
     * when `unit` is 'km' (rounded, no unit suffix — see
