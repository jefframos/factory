...
export class DomUiRoot {
    ...
    /** Full-viewport blocker appended to viewportRoot (NOT scaledRoot — see setInputBlocked()) whenever some real-DOM UI (mute/settings/quit buttons — see SoundToggleButton.ts) needs to stop responding to clicks. These buttons live outside the Pixi canvas entirely, so a Pixi-side blocker sprite (e.g. IslandViewScene's gameBlocker) can never reach them — this is the DOM-side equivalent. */
     ...
     * platform-paused (see IslandViewScene._onPlatformPause/_onPlatformResume). Sits ABOVE
