...
export class ConfirmationPopup extends BasePopup {
    ...
    constructor() {
        ...
        const width = 500;
        const height = 400;
        this.background.beginFill(Assets.Textures.UI.BlockerColor, 0.9);
        ...
        this.cancelButton = new BaseButton({
            standard: {
                ...
                fontStyle: new PIXI.TextStyle({ ...Assets.MainFont, fontSize: 24 }),
            },
            click: {
                callback: () => {
                    this.onCancel?.();
    ...
    async transitionIn(data?: ConfirmationPopupData): Promise<void> {
        ...
        Assets.tryToPlaySound(Assets.Sounds.UI.OpenPopup)
    ...
    async transitionOut(): Promise<void> {
        Assets.tryToPlaySound(Assets.Sounds.UI.ClosePopup)
